...
from prompt_templates import EVALUATION_PROMPT_TEMPLATE, GENERATION_PROMPT_TEMPLATE
...
def media_items(job, kind):
    if kind == "inputs":
        return job.get("input_images") or []
...
def api_preflight():
    ...
    if launched and ports_online:
        message = "调试 Chrome 已启动。请确认 ChatGPT 和 Grok 均已登录，然后再次启动任务。"
...
def api_create_job():
    ...
    if not files:
        return json_error("请上传至少一张参考图片。")
    if len(files) > 3:
        return json_error("最多上传 3 张参考图片。")
    ...
    try:
        for index, storage in enumerate(files, start=1):
            original_name = storage.filename or "image_{0}.jpg".format(index)
            suffix = Path(original_name).suffix.lower()
            if suffix not in allowed_suffixes:
                raise ValueError("不支持的图片格式: {0}".format(original_name))
